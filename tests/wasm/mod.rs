...
prot = "tcp""#,
    ...
    let server = thread::Builder::new()
        .name("server".into())
        .spawn(move || {
            let (mut stream_pre, _) = listener_pre
                .accept()
                .expect("failed to accept preconfigured TCP connection");
            writeln!(stream_pre, "{port_run}").expect("failed to send runtime connection port");
            let (stream_run, _) = listener_run
                .accept()
                .expect("failed to accept runtime TCP connection");

            assert_stream(stream_pre).expect("failed to assert preconfigured TCP stream");
            assert_stream(stream_run).expect("failed to assert runtime TCP stream");
        })
...
fn listen_tcp() -> anyhow::Result<()> {
    ...
    let listener =
        TcpListener::bind((Ipv4Addr::UNSPECIFIED, 0)).context("failed to start TCP listener")?;
    let port_pre = TcpListener::bind((Ipv4Addr::UNSPECIFIED, 0))
...
prot = "tcp""#,
    ...
    let client = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            println!("workload has started (ping received)");

            let port_run = TcpListener::bind((Ipv4Addr::UNSPECIFIED, 0))
                .expect("failed to start TCP listener")
                .local_addr()
                .expect("failed to query listener local address")
                .port();
            stream
                .write_all(format!("{port_run}").as_bytes())
                .expect("failed to send runtime listening port");
            stream
            ...
            assert_connect(connect(port_pre))
                .expect("failed to assert preconfigured TCP connection");
            assert_connect(connect(port_run)).expect("failed to assert runtime TCP connection");
        })
        ...
        .spawn(move || {
            ...
            let connect = |port| {
                move || {
                    ...
                    let tls = rustls::ClientConnection::new(tls, "localhost".try_into().unwrap())
                        .context("failed to create TLS connection")?;
                    anyhow::Result::Ok(rustls::StreamOwned::new(tls, stream))
                }
            };
            assert_connect(connect(port_pre))
                .expect("failed to assert preconfigured TLS connection");
            assert_connect(connect(port_run)).expect("failed to assert runtime TLS connection");
        })
